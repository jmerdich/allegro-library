...
class PagSelectPart(PagSelectBase):
	...
	def getSelectOptionsPanel(self):
		...
		##
		lbl = SortLabel(panel)
		lbl.Caption = biz.getColCaption("partid")
		lbl.relatedDataField = "partid"
		...
		if ctrlClass is not None:
			...
			self.selectFields["partid"] = {
					"ctrl" : ctrl,
					"op" : opList,
					"type": "int"
					}
		else:
			dabo.log.error("No control class found for field 'partid'.")
			lbl.release()
			opList.release()
		...
		##
		lbl = SortLabel(panel)
		lbl.Caption = biz.getColCaption("footprintid")
		lbl.relatedDataField = "footprintid"
		...
		if ctrlClass is not None:
			...
			# Store the info for later use when constructing the query
			self.selectFields["footprintid"] = {
					"ctrl" : ctrl,
					"op" : opList,
					"type": "int"
					}
		else:
			dabo.log.error("No control class found for field 'footprintid'.")
			lbl.release()
			opList.release()
		...
		##
		lbl = SortLabel(panel)
		lbl.Caption = biz.getColCaption("symbolid")
		lbl.relatedDataField = "symbolid"
		...
		if ctrlClass is not None:
			...
			# Store the info for later use when constructing the query
			self.selectFields["symbolid"] = {
					"ctrl" : ctrl,
					"op" : opList,
					"type": "int"
					}
		else:
			dabo.log.error("No control class found for field 'symbolid'.")
			lbl.release()
			opList.release()
		...
		##
		lbl = SortLabel(panel)
		lbl.Caption = biz.getColCaption("verificationstatus")
		lbl.relatedDataField = "verificationstatus"
		...
		if ctrlClass is not None:
			...
			# Store the info for later use when constructing the query
			self.selectFields["verificationstatus"] = {
					"ctrl" : ctrl,
					"op" : opList,
					"type": "int"
					}
		else:
			dabo.log.error("No control class found for field 'verificationstatus'.")
			lbl.release()
			opList.release()
...
if __name__ == "__main__":
	...
	frm = TestForm(Caption="Test Of PagSelectPart", Testing=True)
	test = PagSelectPart(frm)
	test.createItems()
