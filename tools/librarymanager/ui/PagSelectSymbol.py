...
class PagSelectSymbol(PagSelectBase):
	...
	def getSelectOptionsPanel(self):
		...
		##
		lbl = SortLabel(panel)
		lbl.Caption = biz.getColCaption("library")
		lbl.relatedDataField = "library"
		...
		if ctrlClass is not None:
			...
			self.selectFields["library"] = {
					"ctrl" : ctrl,
					"op" : opList,
					"type": "char"
					}
		else:
			dabo.log.error("No control class found for field 'library'.")
			lbl.release()
			opList.release()
...
if __name__ == "__main__":
	...
	frm = TestForm(Caption="Test Of PagSelectSymbol", Testing=True)
	test = PagSelectSymbol(frm)
	test.createItems()
