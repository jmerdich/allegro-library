...
class PagSelectFootprint(PagSelectBase):
	...
	def getSelectOptionsPanel(self):
		...
		##
		lbl = SortLabel(panel)
		lbl.Caption = biz.getColCaption("padsid")
		lbl.relatedDataField = "padsid"
		...
		if ctrlClass is not None:
			...
			# Store the info for later use when constructing the query
			self.selectFields["padsid"] = {
					"ctrl" : ctrl,
					"op" : opList,
					"type": "int"
					}
		else:
			dabo.log.error("No control class found for field 'padsid'.")
			lbl.release()
			opList.release()
...
if __name__ == "__main__":
	...
	frm = TestForm(Caption="Test Of PagSelectFootprint", Testing=True)
	test = PagSelectFootprint(frm)
	test.createItems()
