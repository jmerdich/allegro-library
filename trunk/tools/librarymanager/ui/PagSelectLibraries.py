...
class PagSelectLibraries(PagSelectBase):
	...
	def getSelectOptionsPanel(self):
		...
		##
		lbl = SortLabel(panel)
		lbl.Caption = biz.getColCaption("path")
		lbl.relatedDataField = "path"
		...
		if ctrlClass is not None:
			...
			# Store the info for later use when constructing the query
			self.selectFields["path"] = {
					"ctrl" : ctrl,
					"op" : opList,
					"type": "char"
					}
		else:
			dabo.log.error("No control class found for field 'path'.")
			lbl.release()
			opList.release()
...
if __name__ == "__main__":
	...
	frm = TestForm(Caption="Test Of PagSelectLibraries", Testing=True)
	test = PagSelectLibraries(frm)
	test.createItems()
