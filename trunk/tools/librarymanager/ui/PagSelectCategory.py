...
class PagSelectCategory(PagSelectBase):
	...
	def getSelectOptionsPanel(self):
		...
		##
		lbl = SortLabel(panel)
		lbl.Caption = biz.getColCaption("catid")
		lbl.relatedDataField = "catid"
		...
		if ctrlClass is not None:
			...
			# Store the info for later use when constructing the query
			self.selectFields["catid"] = {
					"ctrl" : ctrl,
					"op" : opList,
					"type": "int"
					}
		else:
			dabo.log.error("No control class found for field 'catid'.")
			lbl.release()
			opList.release()
...
if __name__ == "__main__":
	...
	frm = TestForm(Caption="Test Of PagSelectCategory", Testing=True)
	test = PagSelectCategory(frm)
	test.createItems()
