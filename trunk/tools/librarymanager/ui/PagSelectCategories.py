...
class PagSelectCategories(PagSelectBase):
	...
	def getSelectOptionsPanel(self):
		...
		##
		lbl = SortLabel(panel)
		lbl.Caption = biz.getColCaption("master")
		lbl.relatedDataField = "master"
		...
		if ctrlClass is not None:
			...
			# Store the info for later use when constructing the query
			self.selectFields["master"] = {
					"ctrl" : ctrl,
					"op" : opList,
					"type": "int"
					}
		else:
			dabo.log.error("No control class found for field 'master'.")
			lbl.release()
			opList.release()
...
if __name__ == "__main__":
	...
	frm = TestForm(Caption="Test Of PagSelectCategories", Testing=True)
	test = PagSelectCategories(frm)
	test.createItems()
